...
#
# Что делает: находит САМЫЙ свежий канонический дамп данных
# (townhouse_ГГГГММДД_ЧЧММСС.sql — их создаёт dump_to_sync.sh), сравнивает с
# маркером последнего применённого дампа (.git/db_refresh.state) и, если появился
# новый, полностью пересоздаёт БД (restore_townhouse.sh all --fresh), затем
# догоняет схему/справочники/админа до кода (alembic + init_data + create_user).
# ============================================================================
...
#
# Берём ТОЛЬКО канонические дампы данных — ровно те, что создаёт dump_to_sync.sh
# (townhouse_ГГГГММДД_ЧЧММСС.sql). Прочие файлы в синхронизируемой папке тоже
# подходят под townhouse_*.sql, но дампом текущего состояния НЕ являются:
#   townhouse_roles_*    — роли (импортируются отдельно, не данные);
#   townhouse_pre_*      — точки отката, снятые ПЕРЕД правкой данных;
#   townhouse_for_prod_* — выгрузка, подготовленная для прод-переноса.
# Раньше отбор шёл как `ls | sort | tail -1`, и служебные имена сортировались
# ПОСЛЕ дампов данных ('f'/'p' > цифр) — зеркало подхватывало точку отката вместо
# свежего состояния, а маркер вставал на служебный файл и навсегда блокировал
# дальнейшее обновление (этот же файл и дальше выбирался бы `tail -1`).
DATA="$(ls "$SRC_DIR"/townhouse_*.sql 2>/dev/null | grep -E '/townhouse_[0-9]{8}_[0-9]{6}\.sql$' | sort | tail -1 || true)"
if [ -z "${DATA:-}" ]; then
  echo "⚠️  В '$SRC_DIR' нет дампов данных townhouse_ГГГГММДД_ЧЧММСС.sql — жду первой синхронизации."
  exit 0
fi
