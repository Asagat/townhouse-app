...
//   2) Блок «Движения»: крупно — название движения (денежные документы — по
//      статье доходов/расходов, начисления — по виду услуги), мелко — вид/название
//      документа («Начисление» / «Приход в кассу №2100 от 10.03.2026»);
...
const MOVEMENTS_RESPONSE = {
    ...
    movements: [
        ...
        },
        {
            ...
            article: "Поступления от жителей",
            amount: -500,
            balance_after: 500,
            document: "Приход в кассу №2100 от 10.03.2026",
        },
    ],
...
describe("CabinetView", () => {
    ...
    it("блок «Движения»: денежные движения — по статье, начисления — по услуге", async () => {
        renderView();

        // Заголовок и содержимое загруженного ответа /me/movements.
        expect(await screen.findByText("Движения")).toBeTruthy();

        // Начисление: крупно — вид услуги (статьи у начисления нет), мелко — подпись документа.
        expect(await screen.findByText(/01\.03\.2026 — Вывоз мусора/u)).toBeTruthy();
        expect(await screen.findByText("Начисление")).toBeTruthy();

        // Денежное движение: крупно — статья доходов/расходов (а НЕ услуга, на которую
        // ушли деньги), мелко — вид денежного документа.
        expect(await screen.findByText(/10\.03\.2026 — Поступления от жителей/u)).toBeTruthy();
        expect(await screen.findByText("Приход в кассу №2100 от 10.03.2026")).toBeTruthy();
