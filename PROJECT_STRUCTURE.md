...
| Сентябрь 2026 | **Б10 — общий механизм фильтрации списков** — бэкенд `backend/filtering.py`: серверная фильтрация generic GET по параметрам `<field>[_{like|ne|gte|lte}]=<value>` (разрешение полей переиспользует `sorting.SORT_FIELDS`: прямые столбцы, вложенные поля, агрегаты, автор; автотипизация bool/числа/даты, contains — ILIKE); фронт: кнопка «Фильтры» в `GenericList` (`config/filters.ts`) с контролами по типу колонки (текст/диапазон от-до/период/Да-Нет/выбор), применение через refine `setFilters`; тесты `backend/tests/test_filtering.py`. |
