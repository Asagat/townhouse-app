...
> см. §11 (CI/CD: автопроверка и деплой). `scripts/deploy_vps.sh` дополнительно
...
локальный clone старый), обычный `git pull` покажет рассинхрон/откажет. Приведите
клон один раз к новой истории командой:

```bash
cd <ваш клон townhouse-app>
git fetch origin
git reset --hard origin/main   # не отдельный git pull — истории разошлись
./scripts/update_db_after_pull.sh   # при желании догнать БД (схему/дампы)
```

Пометка про остальные случаи рассинхрона клонов — см. §7.4.
...
Вместо конкретной версии можно поставить `TOWNHOUSE_TAG=latest` — это «последний
выпущенный релиз»: `latest` двигается только релизными тегами `v*`, push в `main`
его не трогает. Тогда обновление = `pull` + `up -d` без правки `.env` (кнопки
обновления в Container Manager нет — см. п.6).
...
#### 6) Обновление версии (только из CLI)

> **Кнопки «Обновить» в Container Manager нет.** У проекта доступны лишь «Перезапуск»
> и «Очистить», и они НЕ пересоздают контейнеры из нового образа (`Перезапуск` поднимает
> тот же контейнер со старым образом). ⚠️ **«Очистить» не использовать** — оно удаляет
> контейнеры проекта и может унести volume `townhouse_pgdata` с данными БД
...
3. Выкат — **`pull` обязателен** (иначе `up -d` поднимет старый локальный образ при том же
   ...
   ```bash
   cd /volume1/docker/townhouse-app
...
что в §9. Папку `backups/` в git не коммитить.

#### 8) Документация API (/docs) — закрыта в проде

Swagger UI (`/docs`), ReDoc (`/redoc`) и схема `/openapi.json` отключены в проде:
они раскрывают структуру API (ресурсы, поля, параметры) любому, кто дошёл до домена.

Управление — переменной `ENABLE_DOCS` (по умолчанию **выключено** — безопасный
дефолт, в проде не откроется по недосмотру):

| Значение | /docs, /redoc, /openapi.json | Где |
|---|---|---|
| не задано / `false` | ❌ 404 | прод (`docker-compose.prod.yml`) |
| `true` | ✅ доступны | локальный dev, демо |

Для разовой отладки на NAS: поставить `ENABLE_DOCS=true` в `.env`, `up -d backend`,
после отладки вернуть `false`. В демо-контуре флаг оставлен `true` (демонстрация API).

**Частые проблемы (NAS):**

| Симптом | Причина / решение |
|---|---|
| `/docs` отдаёт 404 | так и задумано в проде; для отладки `ENABLE_DOCS=true` в `.env` |
...
## 8. Демо-контур (показательная копия)
...
## 9. Резервное копирование БД (дамп)
...
### 9.1 Дамп штатного состояния (полный + роли) для переноса на другой ПК
...
## 11. CI/CD: автопроверка и подготовка образов (GitHub Actions)
...
### 11.4 Тестовый сервер для сверки данных
...
### 11.6 Статус настройки на GitHub (09–08.2026) — памятка, с чего продолжить
