...
async def update_accrual_document_full(
    ...
    if not document:
        raise HTTPException(status_code=404, detail="Документ не найден")
    # Персональные документы правит/просматривает пользователь как историю — не через
    # месячный расчёт (PUT full).
    if document.doc_kind == "oneoff":
        raise HTTPException(
            status_code=422,
            detail="Это персональный документ — он недоступен для месячного пересчёта; "
            "для правки используйте раздел персональных начислений.",
        )
...
async def update_accrual_document_amounts(
    ...
    """Правит строки персонального («персонального/одноразового») документа.

    В отличие от «Начисление за месяц» (PUT .../full), здесь НЕ идёт пересчёт по
    общим ставкам: персональные начисления заданы по конкретным л/с фиксированными
    ...
    if document.doc_kind != "oneoff":
        raise HTTPException(
            status_code=422,
            detail="Этот документ — не персональный (oneoff); для месячного начисления используйте "
