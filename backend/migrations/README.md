...
## Демонстрационные данные (`seed_demo_data.py`)

Наполняет **чистую** (демо-)БД правдоподобным вымышленным набором, чтобы показать
систему целиком: 10 собственников с казахстанскими ФИО (первый — «Мыркынбаева
Мыркынбая»), квартиры/ЛС `LS-0001…LS-0010`, счётчики, показания, начисления,
квитанции, касса и производные регистры. Суммы и ставки «похожи на реальные»
(э/э 40 ₸/кВт·ч; фиксированные — см. `SERVICES` в скрипте); числа детерминированы
(`--seed`), поэтому повторный прогон даёт то же самое.

Все данные создаются **штатным путём приложения** («документ → регистр»):
справочники → тарифы → контрагенты/квартиры/ЛС/счётчики → документы показаний
(базовый замер + по одному на месяц: потребление = разность показаний) → документы
начислений и строки `accruals_register` (`build_accrual_register_items`) → документы
«Приход/Расход» и `cash_register` (событие `transaction_after_insert`) → пересборка
`accounts_register` (`rebuild_accounts_register`, распределение по приоритету услуг) →
квитанции (`generate_receipt_document`) → контроль `check_register_integrity`.
Прямой SQL для вставки данных не используется.

```bash
python migrations/seed_demo_data.py                          # план (dry-run, ничего не пишет)
python migrations/seed_demo_data.py --apply                  # создать данные (только в пустую БД)
python migrations/seed_demo_data.py --apply --wipe           # пересоздать (удалив бизнес-данные)
python migrations/seed_demo_data.py --apply --residents      # + демо-логины ЛК (demo001…, пароль demo12345)
```

Опции: `--months N` (3), `--apartments N` (10), `--end ГГГГ-ММ` (последний
завершённый месяц), `--seed N`, `--no-payments` (без кассы).

Безопасность: по умолчанию — сухой прогон; запись только с `--apply`
(и только в пустую БД — иначе `rc=2` с перечнем найденного); `--wipe` удаляет
бизнес-данные (таблицы из `BUSINESS_TABLES`/`WIPE_ORDER`) и демо-жителей, но не
справочники/пользователей-сотрудников. Перед записью печатается целевая БД
(пароль маскирован).

Демо-логины жителей (`--residents`) привязываются к ЛС; сотрудников (admin/operator)
заводит обычный `create_user.py` — его лучше запустить ДО генерации, тогда
документы получат корректные аудит-поля (`created_by`).
